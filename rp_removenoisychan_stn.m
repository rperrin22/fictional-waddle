function rp_removenoisychan_stn(stn_in_filename,stn_out_filename,var)
...
%% despike the dataset
st_dev = zeros(size(datamat,2)-1,1);
for count = 1:size(datamat,2)-1
st_dev(count) = std(datamat(:,count));
end

st_dev_med = median(st_dev);

datamat(:,st_dev>(var*st_dev_med)) = 0;
